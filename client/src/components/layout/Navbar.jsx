          ...
          <div className="flex justify-between items-center h-20">
            <div className="flex items-center gap-2">
              <Link to={user ? "/dashboard" : "/"} className="flex items-center gap-3 group">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-green-600 to-green-400 flex items-center justify-center text-white font-black shadow-green-500/30 shadow-lg group-hover:scale-105 group-active:scale-95 transition-all duration-300 relative overflow-hidden">
                  <div className="absolute inset-0 bg-white/20 w-full h-1/2 rounded-t-xl" />
                  <span className="relative z-10 text-lg">O</span>
                </div>
                <span className="font-extrabold text-2xl tracking-tighter text-slate-800 group-hover:text-green-600 transition-colors">OsusuApp</span>
              </Link>
            </div>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link 
                    key={link.name} 
                    to={link.path} 
                    className={`relative text-sm font-bold transition-colors ${isActive ? 'text-green-600' : 'text-slate-600 hover:text-green-600'}`}
                  >
                    {link.name}
                    {isActive && (
                      <span className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 bg-green-600 rounded-full"></span>
                    )}
                  </Link>
                );
              })}
          ...
          <div className="p-6 space-y-6 mt-16">
            {navLinks.map((link) => {
              ...
              return (
                <Link 
                  ...
                  className={`block text-lg font-bold ${isActive ? 'text-green-600' : 'text-slate-600'}`}
                >
                  <div className="flex items-center">
                    {link.name}
                    {isActive && <span className="ml-2 w-2 h-2 bg-green-600 rounded-full"></span>}
                  </div>
                </Link>
              );
            })}
            {user && (
              <div className="pt-6 border-t border-slate-200">
                <button onClick={() => { logout(); setMenuOpen(false); }} className="text-lg font-bold text-rose-500 flex items-center">
